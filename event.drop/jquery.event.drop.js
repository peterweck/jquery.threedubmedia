...
$.drop = function ( opts ) {
    ...
    drop.defaults.multi = opts.multi === true ? Infinity :
        opts.multi === false ? 1 : !isNaN( opts.multi ) ? opts.multi : drop.defaults.multi;
    drop.defaults.delay = opts.delay || drop.defaults.delay;
    drop.defaults.tolerance = $.isFunction( opts.tolerance ) ? opts.tolerance :
        opts.tolerance === null ? null : drop.defaults.tolerance;
    drop.defaults.mode = opts.mode || drop.defaults.mode || 'intersect';
};
...
var drop = $special.drop = {
    ...
    defaults: {
        ...
        tolerance: false    // NULL or the drop.modes[mode] function
    },
    ...
    add: function ( obj ) {
        ...
        $.each( drop.defaults, function ( key, def ) {
    ...
    handler: function ( event, dd ) {
        ...
        switch ( event.type ) {
        // dropinit, from $.event.special.drop
        ...
        case 'MSPointerMove': // TOLERATE >>
            ...
            break;
        // dropend, from $.event.special.drop
    ...
    // sort drop target cache by by winner (desc), then index (asc)
    sort: function ( a, b ) {
    ...
    tolerate: function ( dd ) {
        ...
        tolerance = drop.defaults.tolerance || drop.modes[ drop.defaults.mode ];
        // go through each passed interaction...
        do {
            // check valid interaction
            ia = dd.interactions[x];
            if ( ia ) {
                ...
                do {
                    ...
                    if ( data ) {
                        // winners...
                        if ( data.winner && ia.drop.length < drop.defaults.multi ) {
